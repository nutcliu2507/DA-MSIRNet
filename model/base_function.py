...
def print_network(net):
    """print the network"""
    num_params = sum(param.numel() for param in net.parameters())  # 計算參數數量

    x = torch.randn(1, 3, 256, 256)  # Input tensor
    mask = torch.ones(1, 1, 256, 256)  # Mask tensor

    # 使用 fvcore 計算 FLOPs，然後轉換為 MACs
    if net.__class__.__name__ == "Discriminator":
        flops = FlopCountAnalysis(net, x)
    else:
        flops = FlopCountAnalysis(net, (x, mask))
